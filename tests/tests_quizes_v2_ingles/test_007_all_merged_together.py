...
class TestTTSAudio(BaseTest):
    TOTAL_FILES_PER_QUIZ = 2
    TOTAL_QUIZES = 29  # Should match the CSV
    ...
    def test_000_cleanup_files(self):
        ...
        pages_vid_folder = str(Path(self.config["Quizes_v2"]["video_quiz_pages_folder"]))
        audio_path = str(Path(self.config["Quizes_v2"]["audio_to_join"]).parent)
        non_audio_videos = str(Path(self.config["Quizes_v2"]["video_to_join"]).parent)
        final_videos = str(Path(self.config["Quizes_v2"]["video_output_join"]).parent)
    ...
    def test_001_generate_all_audio_files(self):
        ...
        quizes = read_csv(self.config["Quizes_v2"]["input_multiple_csv"])
        audio_path = lambda x: self.config["Quizes_v2"]["audio_to_join"].replace("*", str(x))
    ...
    def test_003_join_quiz_video_final(self):
        ...
        total_quizes = 10
        audio_path = lambda x: self.config["Quizes_v2"]["audio_to_join"].replace("*", str(x))
        video_path = lambda x: self.config["Quizes_v2"]["video_to_join"].replace("*", str(x))
        dst_path = lambda x: self.config["Quizes_v2"]["video_output_join"].replace("*", str(x)).replace("&", "#")
    ...
    def generate_quiz_audio(self, preguntas: list, output_final_audio: str):
        ...
        nivel_facil = self.config["Quizes_v2"]["facil_audio"]
        end_facil = self.config["Quizes_v2"]["end_facil_audio"]
        nivel_medio = self.config["Quizes_v2"]["medio_audio"]
        nivel_dificil = self.config["Quizes_v2"]["dificil_audio"]
        ultima_pregunta = self.config["Quizes_v2"]["ultima_audio"]
        end_audio = self.config["Quizes_v2"]["end_audio"]
    ...
    def _join_quiz_pages(self, start_page, files_per_quiz, video_quiz_count):
        ...
        video_path = lambda x: self.config["Quizes_v2"]["video_to_join"].replace("*", str(x))
        pages_path = lambda x: self.config["Quizes_v2"]["video_quiz_pages"].replace("*", str(x))
        list_of_videos = list()
    ...
    def _unzip_pages(self):
        zip_folder = self.config["Quizes_v2"]["zip_filepath"]
        zip_filename = self.config["Quizes_v2"]["zip_filename"]
        dst_folder = self.config["Quizes_v2"]["video_quiz_pages_folder"]
