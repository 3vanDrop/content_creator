...
class TestTTSAudio(BaseTest):
    ...
    TOTAL_QUIZES = TOTAL_QUIZES  # Should match the CSV
    ...
    def test_001_generate_all_audio_files(self):
        ...
        quizes = read_csv(self.config[CONFIG_YAML_KEY]["input_multiple_csv"])
    ...
    def test_003_join_quiz_video_final(self):
        ...
        dst_path = lambda x: self.config[CONFIG_YAML_KEY]["video_output_join"].replace("*", str(x)).replace("&", "#")
    ...
    def generate_quiz_audio(self, preguntas: list, output_final_audio: str):
        intro_audio = self.config[CONFIG_YAML_KEY]["intro_audio"]
        nivel_facil = self.config[CONFIG_YAML_KEY]["facil_audio"]
        end_facil = self.config[CONFIG_YAML_KEY]["end_facil_audio"]
        nivel_medio = self.config[CONFIG_YAML_KEY]["medio_audio"]
        nivel_dificil = self.config[CONFIG_YAML_KEY]["dificil_audio"]
        ultima_pregunta = self.config[CONFIG_YAML_KEY]["ultima_audio"]
        end_audio = self.config[CONFIG_YAML_KEY]["end_audio"]
    ...
    def _join_quiz_pages(self, start_page, files_per_quiz, video_quiz_count):
        ...
        video_path = lambda x: self.config[CONFIG_YAML_KEY]["video_to_join"].replace("*", str(x))
        pages_path = lambda x: self.config[CONFIG_YAML_KEY]["video_quiz_pages"].replace("*", str(x))
        list_of_videos = list()
    ...
    def _unzip_pages(self):
        zip_folder = self.config[CONFIG_YAML_KEY]["zip_filepath"]
        zip_filename = self.config[CONFIG_YAML_KEY]["zip_filename"]
        dst_folder = self.config[CONFIG_YAML_KEY]["video_quiz_pages_folder"]
