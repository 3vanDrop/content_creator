...
class TestTTSAudio(BaseTest):
    ...
    def test_000_cleanup_files(self):
        ...
        pages_vid_folder = str(Path(self.config["Quizes_v2_Frances"]["video_quiz_pages_folder"]))
        audio_path = str(Path(self.config["Quizes_v2_Frances"]["audio_to_join"]).parent)
        non_audio_videos = str(Path(self.config["Quizes_v2_Frances"]["video_to_join"]).parent)
        final_videos = str(Path(self.config["Quizes_v2_Frances"]["video_output_join"]).parent)
    ...
    def test_003_join_quiz_video_final(self):
        ...
        total_quizes = self.TOTAL_QUIZES
        audio_path = lambda x: self.config["Quizes_v2_Frances"]["audio_to_join"].replace("*", str(x))
        video_path = lambda x: self.config["Quizes_v2_Frances"]["video_to_join"].replace("*", str(x))
        dst_path = lambda x: self.config["Quizes_v2_Frances"]["video_output_join"].replace("*", str(x)).replace("&", "#")
    ...
    def _join_quiz_pages(self, start_page, files_per_quiz, video_quiz_count):
        ...
        video_path = lambda x: self.config["Quizes_v2_Frances"]["video_to_join"].replace("*", str(x))
        pages_path = lambda x: self.config["Quizes_v2_Frances"]["video_quiz_pages"].replace("*", str(x))
        list_of_videos = list()
    ...
    def _unzip_pages(self):
        zip_folder = self.config["Quizes_v2_Frances"]["zip_filepath"]
        zip_filename = self.config["Quizes_v2_Frances"]["zip_filename"]
        dst_folder = self.config["Quizes_v2_Frances"]["video_quiz_pages_folder"]
