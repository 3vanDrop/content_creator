...
class TestBaseVoices(BaseTest):
    ...
    def test_siguiente_pregunta(self):
        audio_path1 = edgetts_generar_voz("¡Siguiente pregunta...!",
                                            voz=self.voz,
                                            expected_duration_ms=1900,
                                          output_file="siguiente.mp3")
        self.logger.info(f"siguiente duration={len(AudioSegment.from_file(audio_path1))}")
